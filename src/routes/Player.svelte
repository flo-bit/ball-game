<script lang="ts">
	...
	*/
    ...
    `
		...
		// Inject the noise function at the top of the fragment shader
		shader.fragmentShader = 'varying vec3 vPos;\n' + noiseFunction + shader.fragmentShader;

		shader.fragmentShader = shader.fragmentShader.replace(
			'vec4 diffuseColor = vec4( diffuse, opacity );', // This is a placeholder line found in the existing shader code
			`
		vec4 diffuseColor = vec4(marble(vPos * 1.0), opacity );
        `
		);
	};
</script>
